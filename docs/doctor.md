...
| `artifact_resolver` | Artifact queues or I/O threads saturated, jobs expired, PE metadata missed the admission budget, exceeded size limits, changed identity or had none to validate, or failed file/consumer work; reports every artifact store in one snapshot |
