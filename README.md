...
Rustinel ships a [Nix flake](flake.nix) providing a prebuilt binary package
for `nix run` and `nix build`. Linux is supported on `x86_64-linux` and
`aarch64-linux`.
...
### Use the package in a flake

```nix
{
  inputs.rustinel.url = "github:Karib0u/rustinel";
}
```
...
An overlay (`rustinel.overlays.default`) is also exposed so you can make
`pkgs.rustinel` available in your own NixOS or Home Manager configuration.

### NixOS module

A NixOS module for `services.rustinel` (systemd service, eBPF
capabilities, TOML config generation, rule management) is planned for a
future PR once it has a VM test and usage history. For now, users can wire
up the systemd service themselves using the package and overlay from this
flake.

### Development shell

```bash
nix develop github:Karib0u/rustinel
```
...
The flake packages the prebuilt musl binary from GitHub releases. When a
